...
context.addController('burstMine-plugin-opencl-generator-opencl-view', [
	...
	function($scope, $q, $http, $modal, p_config, p_component) {
		...
		$scope.loadPlatform = function() {
			...
			$http.get(p_config.apiPath + p_config.plugins.apiPath + '/opencl/platforms/' + $scope.data.platform)
		...
		$scope.loadDevice = function() {
			...
			$http.get(p_config.apiPath + p_config.plugins.apiPath + '/opencl/platforms/' + $scope.data.platform + '/devices/' + $scope.data.device)
			.success(function(p_data) {
				$scope.device = p_data;
				defer.resolve();
			})
